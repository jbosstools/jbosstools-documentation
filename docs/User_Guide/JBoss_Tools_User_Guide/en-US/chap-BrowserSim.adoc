...
1.  View local and remote web applications in BrowserSim
2.  Change the appearance of simulated devices, by rotating them and
changing or removing their skins
3.  Check the look and functionality of web pages in different browsers,
such as the default browser or additional synchronized simulated devices
4.  Generate screen captures of simulated devices and their web pages
5.  View the page source of web pages with editors, Firebug Lite and
Weinre
...
1.  `Save` to save as a `.png` file in the default location. The default
...
2.  `Save As` to save as a `.png` file in a location you specify.
3.  `Copy to Clipboard` to copy the graphic for immediate use.
...
window to refresh. The itemizedlists below outline how to create a
...
7.  Complete the appropriate listitem depending on the location of your
resources:
+
...
1.  Make BrowserSim more prominent to use by making it the default
browser, by adding its icon to the global toolbar and creating a
shortcut key for launching it
2.  Extend the functionality of BrowserSim by adding or modify the
simulated devices it provides
3.  Customize the default settings of BrowserSim for large simulated
...
perspective you are using. As detailed in the itemizedlist below, you
can add the BrowserSim toolbar to other perspectives.
...
as described in the itemizedlist below.
...
1.  In the `Name` field, type the name you want to give the device.
2.  In the `Width` and `Height` fields, type the dimensions of the
device window in pixels.
3.  In the `Pixel Ratio` field, type a value for the ratio of CSS pixels
to device pixels.
4.  In the `User Agent` field, type the User Agent string of your
device. Clearing the `User Agent` check box results in the default User
Agent for the BrowserSim browser being used.
+
[NOTE]
====
...
====
5.  From the `Skin` list, select the skin to be used or select `None`.
+
