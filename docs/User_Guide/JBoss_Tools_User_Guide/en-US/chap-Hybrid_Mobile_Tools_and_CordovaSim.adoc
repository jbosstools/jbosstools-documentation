...
through JBoss Central, as detailed in the itemizedlist below.
...
mobile applications, as demonstrated in the itemizedlist below. It
creates a Cordova project with structure compatible with projects
generated by the Cordova command-line interface (CLI).
...
ready-to-sign applications, as detailed in the itemizedlist below.

[IMPORTANT]
====
Android and iOS APIs must be installed and the IDE correctly configured
to use the Android SDK for this itemizedlist to execute successfully.
====
